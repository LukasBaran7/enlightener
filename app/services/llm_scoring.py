...
class LLMScoringService:
    ...
    async def fetch_article_from_readwise(self, article_id: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if not data.get("results") or len(data["results"]) == 0:
                raise HTTPException(status_code=404, detail=f"Article {article_id} not found in Readwise")
    ...
    async def generate_llm_score_prompt(self, article: Dict[str, Any]) -> str:
        ...
        content = article.get("content") or article.get("extracted_content") or article.get("summary", "")
        ...
        if content and ("<html" in content.lower() or "<body" in content.lower() or "<div" in content.lower()):
            try:
                ...
                logger.info(f"HTML content processed: {len(article.get('content', ''))} chars -> {len(content)} chars")
            except Exception as e:
                logger.warning(f"Error parsing HTML with BeautifulSoup: {str(e)}. Falling back to regex.")
        ...
        if original_length > max_content_length:
            ...
            logger.info(f"Content truncated: {original_length} chars -> {len(truncated_content)} chars")
        else:
...
"""
    ...
    async def call_anthropic_api(self, prompt: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if "usage" in response_data:
                ...
                logger.info(f"Anthropic API token usage: {input_tokens} input + {output_tokens} output = {total_tokens} total")
    ...
    async def call_openai_api(self, prompt: str) -> Dict[str, Any]:
        ...
        data = {
            "model": self.default_model,
            "messages": [
                {"role": "system", "content": "You are an expert article evaluator who provides detailed analysis of article quality."},
    ...
    async def score_article(self, article_id: str, use_cached: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.warning(f"Failed to fetch from Readwise API: {str(e)}, falling back to database")
            article = await self.get_article_by_id(article_id)
...
async def batch_score_articles(
    ...
    try:
                ...
                "$match": {
                    "$expr": {
                        "$not": {
                            "$in": [
                                "$id", 
                                {"$map": {"input": {"$ifNull": [{"$objectToArray": "$llm_scores"}, []]}, "as": "score", "in": "$$score.k"}}
                            ]
                        }
                    }
                }
        ...
        for article in articles:
            try:
                result = await service.score_article(article.get("id"), use_cached=False)
...
async def compare_llm_with_algorithm(
    ...
    try:
        ...
        result = {
            ...
            "score_difference": abs(llm_score.get("llm_score", 0) - article.get("priority_score", 0))
        }
...
async def get_llm_scoring_stats(
    ...
    try:
        ...
        component_pipeline = [
            {
                "$group": {
                    "_id": None,
                    "avg_information_density": {"$avg": "$component_scores.information_density"},
                    "avg_practical_value": {"$avg": "$component_scores.practical_value"},
                    "avg_depth_of_analysis": {"$avg": "$component_scores.depth_of_analysis"},
        ...
        component_result = await db.llm_scores.aggregate(component_pipeline).to_list(length=1)
        
        if component_result:
            stats["component_averages"] = {
                "information_density": component_result[0].get("avg_information_density", 0),
                "practical_value": component_result[0].get("avg_practical_value", 0),
                "depth_of_analysis": component_result[0].get("avg_depth_of_analysis", 0),
