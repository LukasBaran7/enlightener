...
class ArticleService:
    def __init__(self, db: AsyncIOMotorDatabase):
        self.db = db
        self.reader_api_url = "https://readwise.io/api/v3"
    ...
    async def get_random_articles(
        ...
        with at least half from before 2020"""
        ...
        recently_shown = await self.db.to_read.find(
            {"shown_at": {"$gte": two_hours_ago.strftime("%Y-%m-%dT%H:%M:%S.%f+00:00")}}
        ).distinct("article_id")
        ...
        old_articles_pipeline = [
            {
                "$match": {
                    ...
                    "saved_at": {"$lt": "2020-01-01T00:00:00.000+00:00"},
                }
            },
        ...
        new_articles_pipeline = [
            {
                "$match": {
                    "_id": {"$nin": recently_shown},
                    "saved_at": {"$gte": "2020-01-01T00:00:00.000+00:00"},
                }
            },
        ...
        # If we didn't get enough articles,
        # fill with random ones without date restriction
        if len(articles) < limit:
            remaining = limit - len(articles)
            existing_ids = {article["_id"] for article in articles}
            fallback_pipeline = [
                {"$match": {"_id": {"$nin": list(existing_ids) + recently_shown}}},
        ...
        await self.db.to_read.insert_many(
            [{"article_id": article["_id"], "shown_at": now} for article in articles]
        )

        # Clean up old entries
        await self.db.to_read.delete_many(
            {"shown_at": {"$lt": two_hours_ago.strftime("%Y-%m-%dT%H:%M:%S.%f+00:00")}}
        )
    ...
    async def get_daily_article_counts(self, days: int = 7) -> List[Dict[str, Any]]:
        ...
        while current_date <= end_date:
            ...
            all_dates.append({"date": date_str, "count": date_counts[date_str]})
            current_date += timedelta(days=1)

        return all_dates

    async def get_later_article_counts(self, days: int = 7) -> List[Dict[str, Any]]:
        """Get count of articles added to 'later' collection
    ...
    async def get_collection_counts(self) -> Dict[str, int]:
        ...
        return {"archived_count": archived_count, "later_count": later_count}
    ...
    async def get_curated_articles(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        quick_reads_pipeline = [
            {
                "$match": {
                    "_id": {"$nin": recently_shown},
                    "word_count": {"$gt": 0, "$lt": 1000},  # Combined condition
                }
            },
            {"$sample": {"size": 4}},
        ]
        ...
        common_sites = await self.db.later.aggregate(common_sites_pipeline).to_list(
            length=None
        )
        ...
        quick_reads = await self.db.later.aggregate(quick_reads_pipeline).to_list(
            length=None
        )
        archive_articles = await self.db.later.aggregate(archive_pipeline).to_list(
            length=None
        )
        favorite_sources = await self.db.later.aggregate(
            favorite_sources_pipeline
        ).to_list(length=None)
        ...
        if all_articles:
            ...
            await self.db.to_read.insert_many(
                [
                    {"article_id": article["_id"], "shown_at": now}
...
def process_mongodb_doc(doc: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "created_at" not in doc:
        doc["created_at"] = datetime.now(ZoneInfo("UTC")).strftime(
            "%Y-%m-%dT%H:%M:%S.%f+00:00"
        )
    if "updated_at" not in doc:
        doc["updated_at"] = datetime.now(ZoneInfo("UTC")).strftime(
            "%Y-%m-%dT%H:%M:%S.%f+00:00"
        )
...
async def get_articles(
    ...
    limit: int = Query(
        default=500, ge=1, le=500, description="Number of articles to return"
    ),
    ...
    search: Optional[str] = Query(
        default=None, description="Search in title and summary"
    ),
    days: Optional[int] = Query(
        default=None, description="Get articles from last N days"
    ),
...
async def get_random_later_articles(db: AsyncIOMotorDatabase = Depends(get_database)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving random articles: {str(e)}"
        )


@router.get("/later/curated")
async def get_curated_later_articles(db: AsyncIOMotorDatabase = Depends(get_database)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving curated articles: {str(e)}"
        )
...
async def archive_article(
    ...
    try:
        article_service = ArticleService(db)
        return await article_service.move_to_archived(article_id)
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(
